...
inferTypes cenv (ECond pred tbody fbody) = let e = (newTVr cenv)
                                               epred = (inferTypes (snd e) pred)
                                               etbody = (inferTypes (snd e) tbody)
                                               efbody = (inferTypes (snd e) fbody) in
  if (snd epred) == boolType then (cenv, (snd etbody)) else throw (TypeMismatch (snd epred) boolType)
